...
```bash
python .Codex/skills/recipe-book/scripts/gen_thumbnail.py \
  --prompt "<장면 묘사>" \
  --out "week-1/quests/레시피북/images/<슬러그>-썸네일.jpg"
```

- **API 키는 `.Codex/settings.local.json`의 `env.GEMINI_API_KEY`에 보관돼 있다.** 세션 환경변수로 자동 주입되므로 `export` 할 필요가 없다. 그 파일은 gitignore돼 있다.
- `GEMINI_API_KEY is not set`이 뜨면 세션이 그 설정보다 먼저 시작된 것이다. Codex를 재시작하거나, 그 실행 한 번만 앞에 `GEMINI_API_KEY=$(python -c "import json;print(json.load(open('.Codex/settings.local.json'))['env']['GEMINI_API_KEY'])")` 를 붙인다.
